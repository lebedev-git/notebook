...
async def bump_version() -> None:
    ...
    await repo_query(
        "CREATE type::record(string::concat('_sbl_migrations:', <string>$version)) SET version = $version, applied_at = time::now();",
        {"version": new_version},
    )
...
async def lower_version() -> None:
    ...
    if current_version > 0:
        await repo_query(
            "DELETE type::record(string::concat('_sbl_migrations:', <string>$version));",
